...
std::map<uint32_t, uint32_t> nodeIdToSbsIndex;
...
uint32_t GetSbsDisplayId(uint32_t nodeId);
...
void PrintSbsPositions(NodeContainer smallCellEnbs)
{
    ...
    for (uint32_t i = 0; i < smallCellEnbs.GetN(); ++i)
    {
        ...
        if (sbsMobility)
        {
            ...
            std::cout << "  SBS " << GetSbsDisplayId(nodeId)
                      << " Position: (" << sbsPos.x << ", " 
...
void LogUePositions(Ptr<Node> ueNode, uint32_t imsi) {
    ...
    std::ofstream logFile("ue_positions.log", std::ios_base::app);  
    if (logFile.is_open()) {
...
// Convert NodeId to SBS display number (1-5)
uint32_t GetSbsDisplayId(uint32_t nodeId) {
    if (nodeIdToSbsIndex.find(nodeId) != nodeIdToSbsIndex.end()) {
        return nodeIdToSbsIndex[nodeId];
    }
    return nodeId; 
}
  ...
  for (const auto &entry : sbsEnergyModels)
    {
      ...
      std::cout << "0s: [SBS " << GetSbsDisplayId(nodeId) << "] Initial TxPower set to "
...
Ptr<OpenGymSpace> LteGymEnv::GetObservationSpace()
{
    ...
    uint32_t dim = m_energyModels.size() * 6;  
...
Ptr<OpenGymDataContainer> LteGymEnv::GetObservation()
{
    ...
    double simTimeHours = (Simulator::Now().GetSeconds() / m_simulationTime) * 24.0;
    ...
    for (const auto& [sbsId, model] : m_energyModels) {
        ...
        double localSinr = (sbsSinrAverage.count(sbsId)) ? sbsSinrAverage[sbsId] : 0.0;
        obs.push_back(localSinr);
        obs.push_back(model->IsTransitioning() ? 1.0 : 0.0);
        obs.push_back(simTimeHours);  // NEW: Hour of Day (0-24)
    }
...
void LteGymEnv::CollectEnvData()
{
    ...
    SampleMacroSinr(*globalUeDevs, 3, 3.5);
    ...
    for (const auto& [sbsId, model] : m_energyModels) 
    {
        ...
        std::cout << "  [SBS " << GetSbsDisplayId(sbsId) << "] UEs mapped: ";
        ...
        std::cout << "  [SBS " << GetSbsDisplayId(sbsId) << "] Avg SINR this step: " << sinrAvg << " dB" << std::endl;
    ...
    for (uint32_t i = 0; i < globalUeDevs->GetN(); ++i) 
    {
        ...
        if (ueToSbsMap.count(imsi) && ueToSbsMap[imsi] == 3)
        {
            if (ueActivityMap[imsi]) {
                macroActiveUEs++;
            }
...
double LteGymEnv::ScalePower(double power) {
    const double powerExcellent = 3.36;
    const double powerPoor = 20.7;
...
float LteGymEnv::GetReward()
{
    ...
    const double w_power = 0.3 / numSbs;
    ...
    for (const auto& [sbsId, model] : m_energyModels)
    {
        ...
        m_lastStates[sbsId] = model->GetState();

        std::cout << "[SBS " << GetSbsDisplayId(sbsId) << "] Power=" << power
    ...
    double sinrMultiplier = 0.5 + 0.5 * coverageFactor;
...
bool LteGymEnv::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
    ...
    for (const auto& [sbsId, model] : m_energyModels) {
        ...
        std::cout << Simulator::Now().GetSeconds()
              << "s: [SBS " << GetSbsDisplayId(sbsId) << "] State: " << StateToString(model->GetState())
...
int main(int argc, char *argv[])
{
    ...
    uePositionAlloc->SetAttribute("X", StringValue("ns3::UniformRandomVariable[Min=0.0|Max=2750.0]"));
    ...
    mobility.SetPositionAllocator("ns3::GridPositionAllocator",
        ...
        "DeltaX", DoubleValue(500.0), "GridWidth", UintegerValue(numSmallCells),
        "LayoutType", StringValue("RowFirst"));
    ...
    lteHelper->AddX2Interface(allEnbs);
    ...
    for (uint32_t i = 0; i < smallCellEnbs.GetN(); ++i)
    {
        ...
        scEnergyModel->SetEnergySource(DynamicCast<energy::EnergySource>(energySource));
        ...
        nodeIdToSbsIndex[nodeId] = i + 1;
        std::cout << "Mapped NodeId " << nodeId << " to SBS " << (i + 1) << std::endl;
    }
    ...
    for (uint16_t i = 0; i < allEnbs.GetN(); ++i)
    {
        ...
        Ptr<NetDevice> dev = enbNode->GetDevice(0);
